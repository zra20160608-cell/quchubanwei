...
> **撰写人**: 产品经理  
> **状态**: 评审通过，待设计启动
> **更新说明**: 在v1.1基础上处理23项评审意见，确认P0技术方案，补充设计规范、前端架构、数据库优化、测试策略、接口统一规范等内容
...
```
前端层: 微信小程序（Taro 3.x + React）
  ├── 支持多端输出（微信/支付宝/H5）
  ├── 状态管理：Recoil / Jotai
  ├── UI组件库：自定义+Taro UI
  └── 性能优化：虚拟列表、图片懒加载、代码分包
  ↓
API网关: Kong / Nginx（统一鉴权/限流/日志）
  ↓
业务微服务: Node.js + TypeScript
  ├── Auth Service      // 微信登录、Token管理
  ├── Detect Service    // 检测任务调度、状态机持久化
  ├── AI Proxy Service  // AI服务代理、熔断、缓存、降级
  ├── Plan Service      // 方案管理、打卡追踪
  ├── Social Service    // 帖子、评论、互动
  ├── Notify Service    // 通知推送、消息队列
  ├── Upload Service    // 文件上传、OSS代理（STS临时凭证）
  └── Admin Service     // 内容审核、运营后台
  ↓
AI服务层:
  - 图像识别: 阿里云/腾讯云/百度AI CV API（MVP）→ 后续自研YOLO微调
  - 浓度评分: 规则引擎 + 加权计算（主），LLM微调（辅）
  - 文案生成: GPT-4o / Claude / 文心一言 API（20%场景）+ 模板库（80%场景）
  - 方案推荐: 基于标签匹配 + LLM生成
  - 内容审核: 阿里云内容安全 / 腾讯云天御
  ↓
数据层:
  - 结构化数据: PostgreSQL（主库）+ 读写分离
  - 缓存: Redis（缓存+分布式锁+消息队列Stream）
  - 对象存储: OSS / COS（图片存储，CDN加速）
  - 消息队列: RabbitMQ / Redis Stream（审核/通知/成就/奖励）
  - 冷存储: ClickHouse / Elasticsearch（历史数据归档）
  - 日志/监控: ELK + Prometheus + Grafana
```

### 4.2 AI能力需求与成本控制策略

| 能力 | 方案 | 成本估算 | 可行性 | 成本控制措施 |
|------|------|----------|--------|-------------|
| 图像元素识别 | 调用云厂商CV API（MVP阶段） | ¥0.1-0.5/次 | ✅ 高 | 图片perceptual hash缓存24h，命中率目标40% |
| 浓度评分算法 | 规则引擎+加权计算（主），LLM微调（辅） | 开发成本 | ✅ 高 | 纯规则引擎不走AI，0成本 |
| 辣评文案生成 | 模板库（80%）+ 大模型API（20%） | ¥0.01-0.1/次 | ✅ 高 | 模板命中率≥80%，复杂场景走LLM |
| 方案推荐 | 基于标签匹配（主）+ LLM生成（辅） | ¥0.01-0.05/次 | ✅ 高 | 模板方案为主，个性化仅高活跃用户 |
| 内容审核 | 云厂商内容安全API | ¥0.01/次 | ✅ 高 | 全量走AI预审，无法降本 |

**AI调用熔断与限流策略：**
- 单用户每小时最多3次AI分析（超限走规则引擎）
- 单用户每日最多5次辣评LLM调用（超限走模板库）
- 全站AI服务异常时自动降级：规则引擎评分 + 固定模板辣评
- 图片特征缓存24小时（perceptual hash索引），相同特征直接返回缓存结果

**成本预估（5万DAU）：**
- 日检测量：5万 × 30%（日活检测率）= 1.5万次/天
- AI调用量：1.5万 × 1.5次（识别+评分+文案）= 2.25万次/天
- 缓存命中率40%后实际调用：1.35万次/天
- 日成本：1.35万 × ¥0.15 ≈ ¥2,025/天 ≈ ¥60,750/月
- 模板兜底后LLM调用仅20%：¥60,750 × 20% ≈ ¥12,150/月（纯LLM成本）

### 4.3 风险与应对
| 风险 | 影响 | 应对方案 |
|------|------|----------|
| AI识别准确率不足 | 用户体验差 | MVP阶段聚焦10-15个核心元素识别，逐步扩展；建立黄金数据集100张验证准确率≥85% |
| 大模型API成本高 | 运营亏损 | 模板库命中率≥80%+图片特征缓存+单用户限流；MVP后评估自研YOLO替代云API |
| AI分析延迟过高 | 用户流失 | SSE推送进度+前端伪进度动画（基于历史平均耗时模拟）；支持后台分析+推送通知 |
| 内容审核压力大 | 合规风险 | AI预审≤2秒（P99）+人工复审SLA≤4小时+用户举报机制；消息队列处理审核任务 |
| 图片存储成本高 | 运营成本 | 图片生命周期：原始图30天→缩略图→90天后删除；CDN加速+压缩 |
| OSS直传安全性 | 数据泄露风险 | 采用STS临时凭证方案（有效期5分钟）+后端代理上传备选；小程序域名白名单限制 |
| 分享裂变数据不一致 | 统计失真 | 事件溯源记录完整链路+inviteCode本地缓存fallback+每日对账机制+幂等设计 |
...
- 适配iOS/Android主流机型（最小兼容矩阵：iPhone 12/小米13/华为Mate60）
- 适配微信最新3个版本
- **v1.1即支持深色模式**（建立CSS变量体系，核心页面优先适配）
- 字体大小调节适配（无障碍需求）

### 5.3 安全与隐私
- 用户图片默认不公开，仅在用户授权分享后展示
- 图片存储加密（服务端加密SSE-S3）
- 图片生命周期策略：原始图保留30天→自动转存缩略图→90天后永久删除
- 用户删除账号时级联删除所有图片和数据（符合《个人信息保护法》）
- 敏感信息（如面部特征）采用服务端加密或脱敏存储
- OSS上传采用STS临时凭证（有效期5分钟），避免前端暴露长期凭证
- 符合《个人信息保护法》要求，提供数据导出功能
- 前端接入错误监控（Sentry或微信小程序监控平台）
...
- [x] 热门榜单
- [x] 用户个人主页
- [x] **深色模式适配**（核心页面：检测/报告/圈子/个人中心）
- [x] 设计规范文档（色彩/字体/图标/动效）
- [x] 前端架构搭建（Taro + React + CSS变量体系）
- [x] 接口统一响应格式与Mock Server
- [x] 数据库索引优化与分表策略
- [x] 微服务拆分与消息队列接入
- [x] AI成本控制与降级策略落地
- [x] 测试兼容性矩阵与自动化测试框架
...
| 功能模块 | 验收项 | 通过标准 |
|----------|--------|----------|
| 班味检测 | 图片上传 | 支持拍照+相册，5MB以内，P95≤3秒完成 |
| 班味检测 | AI分析 | P50≤5秒, P95≤8秒, P99≤12秒；识别≥10个核心元素；准确率≥85%（黄金数据集验证） |
| 班味检测 | 报告展示 | 包含浓度评分+雷达图+元素清单+3条辣评 |
| 班味检测 | 分享海报 | 服务端生成，P95≤2秒，海报尺寸≤500KB |
| 去班味方案 | 方案推荐 | 与检测结果关联度≥80%，含行动项+效果+时长 |
| 去班味方案 | 打卡功能 | 支持标记/打卡/查看历史；补卡限3次/方案 |
| 社交互动 | 圈子发布 | 支持图文+话题+匿名，发布后5秒内可见 |
| 社交互动 | 内容审核 | AI预审P99≤2秒；违规拦截率≥95%；人工复审准确率≥95%；复审SLA≤4小时 |
| 社交互动 | 互动功能 | 点赞/评论/收藏操作响应P99≤500ms；并发点赞幂等 |
| 性能 | 页面首屏 | ≤1.5秒 |
| 性能 | 信息流翻页 | P99≤1秒 |
| 性能 | API响应 | P99≤500ms（后端目标P99≤300ms） |
| 安全 | 弱网可用性 | 3G网络下核心功能100%可用 |
| 安全 | 并发安全性 | 100并发下无脏数据 |
| 安全 | 数据一致性 | 多端登录数据延迟≤5秒 |
| 稳定性 | 崩溃率 | ≤0.1%（全渠道） |
| 稳定性 | 内存占用 | 小程序内存峰值≤200MB |
...
**分页规范：**
所有列表接口统一分页参数：`?page=1&limit=20&sort=hot`
统一返回：
```json
{
  "code": 0,
...
### 14.1 前端API列表（v1.2补充4个缺失接口）

#### 用户模块补充
| 接口 | 方法 | 路径 | 参数 | 返回值 | 说明 |
|------|------|------|------|--------|------|
| 获取用户统计 | GET | /api/user/stats | - | {detectionCount, avgScore, streakDays, levelProgress} | 个人首页统计数据 |

#### 帖子模块补充
| 接口 | 方法 | 路径 | 参数 | 返回值 | 说明 |
|------|------|------|------|--------|------|
| 举报帖子 | POST | /api/posts/:id/report | {reason, type} | - | 举报不当内容 |
| 搜索帖子 | GET | /api/search | {q, type, page} | Post[] | 帖子/话题搜索 |

#### 趋势模块
| 接口 | 方法 | 路径 | 参数 | 返回值 | 说明 |
|------|------|------|------|--------|------|
| 获取趋势榜 | GET | /api/trends | {type, period} | TrendItem[] | 班味趋势排行榜 |
...
### 14.3 文件上传接口规范（v1.2修订：改为STS临时凭证方案）

#### 方案决策：STS临时凭证直传（主方案）+ 后端代理上传（备选）

**推荐方案：STS临时凭证直传**
- 后端通过RAM生成STS临时凭证（有效期5分钟）
- 前端使用临时凭证直传OSS，减少服务端带宽压力
- 小程序 `uploadFile` 域名白名单预先配置
- 临时凭证仅拥有特定路径的写权限，最小权限原则

**备选方案：后端代理上传**
- 前端→后端→OSS，牺牲性能换取更高安全性
- 适用于对安全要求极高的场景
...
4. 后端返回STS凭证：
   {
     ...
     "key": "detect/20260430/{userId}_{timestamp}_{random6}.{ext}",
     "bucket": "banwei-oss",
     "region": "oss-cn-hangzhou",
     "endpoint": "https://oss-cn-hangzhou.aliyuncs.com"
   }
5. 前端使用OSS SDK（或直传API）上传文件
   - 携带STS凭证
   - 限制上传路径为返回的key
6. 上传成功后拼接CDN访问URL
   https://cdn.example.com/{key}
7. 调用创建检测任务API，传入图片URL
```
...
> 📌 **PRD v1.2 更新说明**:
> - 在v1.1基础上处理23项评审意见
> - 确认P0技术方案：AI延迟用SSE推送+伪进度动画；成本控制用模板80%+缓存+限流；OSS安全用STS临时凭证
> - 补充前端架构选型（Taro 3.x + React + Recoil）和微服务拆分方案（8个服务）
> - 补充AI成本控制策略与降级方案（单用户日限3次、模板命中率≥80%）
> - 补充数据库索引设计（15个索引）、分表策略（按月分区）、JSON字段拆表评估
> - 补充接口统一规范（统一响应格式、分页、错误码）和4个缺失接口
> - 补充设计规范草案（色彩/字体/图标/动效）
> - 补充测试策略（兼容性矩阵、性能基准P50/P95/P99、安全测试、12个核心用例）
> - 深色模式从v1.3提前到v1.1
> - 补充交互细节：拍照引导、海报多模板、信息流手势
> - **状态**: 评审通过，进入设计与开发阶段
...
:root {
  /* 主色 */
  --color-primary: #FF6B6B;        /* 班味红 - 高分警示、强调 */
  --color-secondary: #4ECDC4;       /* 清新绿 - 去班味方案、正向 */
  --color-accent: #FFE66D;          /* 活力黄 - 打卡激励、成就 */
  
  /* 中性色 */
  --color-text-primary: #2C3E50;    /* 职场深蓝 - 主文字 */
  ...
  /* 班味等级色 */
  --color-level-fresh: #27AE60;     /* 清新脱俗 0-30 */
  --color-level-light: #F39C12;     /* 微醺班味 31-60 */
  --color-level-heavy: #E67E22;     /* 班味浓郁 61-85 */
  --color-level-extreme: #C0392B;   /* 腌入味了 86-100 */
}
...
| 层级 | 字体 | 大小 | 字重 | 说明 |
|------|------|------|------|------|
| 大标题 | 钉钉进步体 | 28px | Bold | 页面主标题 |
| 小标题 | 钉钉进步体 | 20px | Medium | 模块标题 |
| 正文 | 系统默认 | 16px | Regular | 阅读文字 |
| 辅助 | 系统默认 | 14px | Regular | 标签、说明 |
| 辣评文案 | 站酷快乐体/综艺体 | 18px | Bold | 趣味文案 |
| 数据数字 | DIN / Roboto | 48px | Bold | 浓度评分等大数字 |

### 15.3 图标风格

- **班味元素图标**：手绘插画风格，2.5D，略带夸张，圆角处理
- **功能图标**：线性图标，2px描边，圆角端点（24×24px标准尺寸）
- **表情/状态图标**：面性图标，使用主色/辅色填充

### 15.4 动效规范

| 场景 | 动效 | 时长 | 缓动 |
|------|------|------|------|
| 页面转场 | 左右滑动+淡入 | 300ms | cubic-bezier(0.4, 0, 0.2, 1) |
| 数据展示 | 数字滚动 | 800ms | ease-out |
| 交互反馈 | 缩放回弹 | 150ms | spring(1, 80, 10) |
| 加载状态 | 骨架屏渐变 | 持续 | shimmer动画 |
| 打卡成功 | 班味值下降动画 | 1000ms | ease-in-out |
| 点赞 | 心跳缩放+粒子 | 400ms | ease-out |
...
| 设备 | 系统 | 微信版本 | 测试优先级 |
|------|------|----------|-----------|
| iPhone 12/13/14 | iOS 16+ | 最新3版本 | P0 |
| iPhone X/11 | iOS 15+ | 最新3版本 | P1 |
| 小米13/14 | MIUI 14 | 最新3版本 | P0 |
| 华为Mate60 | HarmonyOS 4 | 最新3版本 | P0 |
| OPPO Find X | ColorOS 13 | 最新3版本 | P1 |
| 低端安卓 | Android 10 | 最新3版本 | P2 |

### 16.2 性能测试基准

| 指标 | P50 | P95 | P99 | 测试工具 |
|------|-----|-----|-----|----------|
| 图片上传 | ≤2s | ≤3s | ≤5s | JMeter |
| AI分析 | ≤5s | ≤8s | ≤12s | Artillery |
| 页面首屏 | ≤1s | ≤1.5s | ≤2s | Lighthouse |
| 信息流翻页 | ≤500ms | ≤800ms | ≤1s | Artillery |
| API响应 | ≤100ms | ≤300ms | ≤500ms | JMeter |
| 海报生成 | ≤1s | ≤2s | ≤3s | 自定义脚本 |
...
| 测试项 | 方法 | 通过标准 |
|--------|------|----------|
| XSS注入 | 在帖子/评论中注入脚本标签 | 脚本不被执行，原文展示 |
| SQL注入 | 在接口参数中注入SQL | 返回参数错误，无数据库报错 |
| 越权访问 | 用A用户Token访问B用户数据 | 返回403禁止访问 |
| 文件上传安全 | 上传非图片文件/恶意文件 | 前端拦截+后端二次校验 |
| 接口防刷 | 1000次/分钟单IP请求 | 触发429限流 |
| 敏感数据泄露 | 抓包检查响应体 | 无password/token等敏感字段 |
...
集成测试（Supertest）覆盖100%核心接口
├── 24个API的CRUD+权限验证
├── AI服务Mock测试（降级逻辑）
└── 数据库事务一致性

E2E测试（Miniprogram-Automator）覆盖5条核心流程
├── 拍照→分析→分享（Happy Path）
├── 上传失败→重试→成功
├── 发布帖子→审核→展示
├── 选择方案→打卡→复测
...
### 17.1 拍照引导实时反馈
- 相机取景框overlay动态引导线（"把工位拍全"、"对焦到外卖盒"）
- 拍照后AI预检：检测模糊度、是否含主体（工位/人脸）
- 不合格图片实时提示重拍，避免进入正式分析流程浪费资源

### 17.2 分享海报多模板
- 提供5种海报模板风格：极简/涂鸦/复古/赛博朋克/社畜梗图
- 用户可在AI辣评基础上自定义文案（最多修改20%内容）
- 支持隐藏具体分数（仅展示等级标签，保护隐私）
- 海报底部可选：显示/隐藏用户昵称和头像

### 17.3 圈子信息流手势
- 左滑收藏、右滑分享
- 双击点赞（配合心跳动画）
- 长按帖子唤起快捷操作菜单（点赞/评论/收藏/举报）
- 下拉刷新配合趣味加载文案（"正在打捞职场碎片..."）
- 上滑加载更多配合 Skeleton 骨架屏

### 17.4 分析中页等待体验优化
- AI拟人化角色「班味侦探」：扫描动画跟随识别进度，实时播报
- 趣味职场金句轮播（"正在计算你的打工年限..."）
- 超过5秒未出结果时，展示「后台分析中，可离开小程序」提示
- 支持订阅微信服务通知，分析完成后推送结果

### 17.5 匿名/实名双轨发布
- 发布时可选匿名/实名（默认实名）
- 匿名帖子使用随机生成的「职场动物头像」（如「加班仓鼠」、「摸鱼猫」）
- 匿名帖子热门后可引导作者实名认领，给予额外曝光+成就奖励
- 实名帖子有10%额外曝光权重

---

> 📌 **PRD v1.2 里程碑**: 评审通过 → 设计启动 → 开发启动 → 测试启动 → 上线
> - 设计阶段：2026-05-02 ~ 2026-05-12
> - 开发阶段：2026-05-12 ~ 2026-06-02
> - 测试阶段：2026-06-02 ~ 2026-06-09
> - 上线日期：2026-06-09
